...
class MCPServerRunner:
    ...
    def start_server(
        self, server_name: Optional[str] = None, command: Optional[str] = None
    ) -> bool:
        """
        Start an MCP server.
    ...
    def _get_server_command(self, server_config: Dict[str, Any]) -> Optional[List[str]]:
        ...
        for mcp_cmd in mcp_servers:
            try:
                result = subprocess.run(["which", mcp_cmd], capture_output=True, text=True)
                if result.returncode == 0:
                    # Found command, construct server command
                    if mcp_cmd == "npx":
                        # Try common MCP server packages
                        server_package = config.get(
                            "package", "@modelcontextprotocol/server-browserbase"
                        )
    ...
    def list_running_servers(self) -> List[Dict[str, Any]]:
        ...
        for server_name, proc in self.processes.items():
            if proc.poll() is None:
                server_info = {"name": server_name, "pid": proc.pid, "status": "running"}
    ...
    def check_server_health(self, server_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        proc = self.processes[server_name]
        if proc.poll() is None:
            health = {"status": "healthy", "server": server_name, "pid": proc.pid}
        ...
        if choice == "1":
            ...
            if running:
                ...
                for server in running:
                    cpu_info = (
                        f"CPU: {server.get('cpu_percent', 0):.1f}%"
                        if "cpu_percent" in server
                        else ""
                    )
                    mem_info = (
                        f"Memory: {server.get('memory_mb', 0):.1f} MB"
                        if "memory_mb" in server
                        else ""
                    )
...
if __name__ == "__main__":
    ...
    if args.command == "start":
        ...
        success = runner.stop_server(server_name=getattr(args, "server_name", None))
        sys.exit(0 if success else 1)
        ...
        health = runner.check_server_health(server_name=getattr(args, "server_name", None))
