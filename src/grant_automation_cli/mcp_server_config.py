...
DEVELOPER GUIDELINE: Security Protocol
Provides utilities to configure and connect to MCP servers for browser automation.
Configurations may contain sensitive API keys or local host ports. Do NOT commit 
JSON files generated by this configuration to version control. Validate paths safely.
"""
...
class MCPServerConfig:
    ...
    def _connect_playwright(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                from playwright.sync_api import (  # noqa: F401  # type: ignore
                    Browser,
                    Page,
                    sync_playwright,
                )
    ...
    def _create_playwright_tools(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            def init_playwright():
                ...
                with _playwright_context["lock"]:  # type: ignore
                    if _playwright_context["playwright"] is None:
                        try:
                            ...
                            _playwright_context["browser"] = _playwright_context[
                                "playwright"
                            ].chromium.launch(headless=False)  # type: ignore
                            _playwright_context["page"] = _playwright_context["browser"].new_page()  # type: ignore
            ...
            def playwright_navigate(url: str) -> bool:
                ...
                try:
                    init_playwright()
                    _playwright_context["page"].goto(url)  # type: ignore
            ...
            def playwright_click(selector_or_description: str) -> bool:
                ...
                try:
                    ...
                    if is_likely_selector:
                        try:
                            page.click(selector_or_description, timeout=2000)  # type: ignore
                    ...
                    if "login" in desc_lower or "sign" in desc_lower or "submit" in desc_lower:
                        ...
                        form_submit_approaches = [
                            ...
                            lambda: page.evaluate("""() => {  # type: ignore
                            ...
                            lambda: page.locator("form").first.evaluate("form => form.submit()"),  # type: ignore
                            # Method 3: Press Enter on password field (common form submission)
                            lambda: page.locator('input[type="password"]').first.press("Enter"),  # type: ignore
                        ]
                    ...
                    if (
                        ...
                        approaches = [
                            ...
                            lambda: page.locator('button[type="submit"]').first.click(timeout=3000),  # type: ignore
                            lambda: page.locator('input[type="submit"]').first.click(timeout=3000),  # type: ignore
                            # Try exact button text matches
                            lambda: page.get_by_role("button", name="login", exact=False).click(  # type: ignore
                                timeout=3000
                            ),
                            lambda: page.get_by_role("button", name="sign in", exact=False).click(  # type: ignore
                                timeout=3000
                            ),
                            lambda: page.get_by_role("button", name="Sign In", exact=False).click(  # type: ignore
                                timeout=3000
                            ),
                            lambda: page.get_by_role("button", name="Log In", exact=False).click(  # type: ignore
                                timeout=3000
                            ),
                            lambda: page.get_by_role("button", name="submit", exact=False).click(  # type: ignore
                                timeout=3000
                            ),
                            # Try by text content in button (case insensitive)
                            lambda: (
                                page.locator("button")  # type: ignore
                                .filter(has_text=re.compile(r"login|sign.*in", re.I))
                                .first.click(timeout=3000)  # type: ignore
                            ),
                            # Try with keywords
                            lambda: page.get_by_role(  # type: ignore
                                "button",
                                name=lambda name: (
                                    name
                            ...
                            lambda: page.locator("form button").first.click(timeout=3000),  # type: ignore
                            # Try any button
                            lambda: page.get_by_role("button").first.click(timeout=3000),  # type: ignore
                            lambda: page.locator("button").first.click(timeout=3000),  # type: ignore
                        ]
                    ...
                    approaches = [
                        ...
                        ).first.click(timeout=2000),  # type: ignore
                        lambda: page.get_by_text(selector_or_description, exact=False).first.click(  # type: ignore
                            timeout=2000
                        ),
                        lambda: page.locator(  # type: ignore
                ...
                try:
                    ...
                    if "username" in desc_lower or "email" in desc_lower:
                        ...
                        approaches = [
                            ...
                            lambda: page.get_by_placeholder("Email", exact=False).fill(text),
                            lambda: page.locator('input[type="email"]').first.fill(text),  # type: ignore
                            lambda: page.locator('input[type="text"]').first.fill(text),  # type: ignore
                            lambda: page.locator(  # type: ignore
                    ...
                    if "password" in desc_lower:
                        ...
                        approaches = [
                            ...
                            lambda: page.get_by_placeholder("Password", exact=False).fill(text),
                            lambda: page.locator('input[type="password"]').fill(text),  # type: ignore
                            lambda: page.locator('input[name*="pass" i]').first.fill(text),  # type: ignore
                            lambda: page.locator('input[id*="pass" i]').first.fill(text),  # type: ignore
                        ]
                        for approach in approaches:
                            try:
                                approach()
                    ...
                    try:
                        page.locator("input").first.fill(text)  # type: ignore
