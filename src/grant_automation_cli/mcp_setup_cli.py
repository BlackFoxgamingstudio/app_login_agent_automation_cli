...
def cmd_inject_tools(args):
    ...
    if success:
        ...
        print("  python3 -m grant_automation_cli.sdk_cli auth login")
    else:
        print("❌ Failed to inject tools")
        print("\nTry:")
        print("  1. Configure MCP server: python3 -m grant_automation_cli.mcp_setup_cli configure")
        print("  2. Check server connection: python3 -m grant_automation_cli.mcp_setup_cli test")
...
def cmd_auto_setup(args):
    ...
    if success:
        ...
        print("\nTry manual setup:")
        print("  python3 -m grant_automation_cli.mcp_setup_cli configure")
...
def cmd_status(args):
    ...
    if servers:
        for server in servers:
            ...
            status = (
                "✅ Enabled" if server_config and server_config.get("enabled") else "❌ Disabled"
            )
            print(f"   - {server} ({server_type}) - {status}")
        default = config.config.get("default_server")
        if default:
    ...
    if tools:
        ...
        print("   You can now use: python3 -m grant_automation_cli.sdk_cli auth login")
    else:
        print("   ❌ No MCP tools available")
        print("\n   To enable tools:")
        print("   - Run: python3 -m grant_automation_cli.mcp_setup_cli auto-setup")
        print("   - Or: python3 -m grant_automation_cli.mcp_setup_cli configure")
...
Examples:
  ...
  python3 -m grant_automation_cli.mcp_setup_cli list
  
  # Test server connection
  python3 -m grant_automation_cli.mcp_setup_cli test
  
  # Inject tools from server
  python3 -m grant_automation_cli.mcp_setup_cli inject
  
  # Auto-setup tools
  python3 -m grant_automation_cli.mcp_setup_cli auto-setup
  
  # Check status
  python3 -m grant_automation_cli.mcp_setup_cli status
        """,
    )
    ...
    def cmd_run_server(args):
        ...
        if success:
            ...
            print("  - Check status: python3 -m grant_automation_cli.mcp_setup_cli status")
            print("  - Inject tools: python3 -m grant_automation_cli.mcp_setup_cli inject")
        else:
