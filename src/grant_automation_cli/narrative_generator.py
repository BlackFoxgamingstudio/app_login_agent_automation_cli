...
"""
    ...
    def _collect_organization_context(self, grant_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "target_audience": grant_data.get("project", {}).get(
                "target_audience", "Not specified"
            ),
            "years_operating": 2024 - org.get("founded", 2024)
            if org.get("founded")
            else "Not specified",
            ...
            "current_infrastructure": grant_data.get("technical", {}).get(
    ...
    def _collect_grant_context(self, grant_name: str, grant_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "matching_funds_percentage": budget.get("matching_funds", {}).get("percentage", 0)
            if isinstance(budget.get("matching_funds"), dict)
            else 0,
        }
    ...
    def _call_openai_api(
        self, prompt: str, system_message: Optional[str] = None, max_retries: int = 3
    ) -> Optional[str]:
        """
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.info(
                    f"Narrative generated in {elapsed_time:.2f}s using {response.usage.total_tokens if response.usage else 'unknown'} tokens"
                )
                ...
                logger.warning(
                    f"OpenAI API rate limit exceeded, waiting {wait_time}s before retry {attempt + 1}/{max_retries}"
                )
                if attempt < max_retries - 1:
                ...
                if "invalid" in error_str and "api key" in error_str:
                    logger.error("Invalid OpenAI API key. Please check your API key.")
                    return None
    ...
    def generate_project_description(
        self, grant_name: str, grant_data: Dict[str, Any]
    ) -> Optional[str]:
        """
    ...
    def generate_goals_narrative(
        ...
        data = {**org_context, **project_context, **grant_context, "goals_list": goals_list}
    ...
    def generate_activities_narrative(
        self, grant_name: str, grant_data: Dict[str, Any]
    ) -> Optional[str]:
    ...
    def generate_narrative(
        self, narrative_type: str, grant_name: str, grant_data: Dict[str, Any]
    ) -> Optional[str]:
        """
