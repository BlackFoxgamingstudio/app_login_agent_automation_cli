...
class GrantSDKCLI:
    ...
    def _get_mcp_browser_tools(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not mcp_tools:
                try:
                    from .inject_mcp_tools import setup_mcp_tools_auto
            ...
            logger.info("  python3 -m grant_automation_cli.mcp_server_config")
            logger.info("")
            logger.info("Method 2: Inject Tools Manually")
            logger.info(
                "  from grant_automation_cli.inject_mcp_tools import inject_tools_from_dict"
            )
            logger.info(
                "  inject_tools_from_dict({'cursor_ide_browser_navigate': your_function, ...})"
            )
            logger.info("")
        ...
        try:
            ...
            if server_type == "playwright":
                ...
                if not server_running:
                    print("⚠️  Playwright MCP server is not running")
                    if non_interactive or self._prompt_yes_no(
                        "Start Playwright MCP server now?", default=True
                    ):
                ...
                if non_interactive or self._prompt_yes_no(
                    "Initialize Playwright browser tools?", default=True
                ):
                    try:
                        ...
                        if tools:
                            ...
                            if mcp_tools:
                                print(
                                    f"✅ Successfully initialized {len(mcp_tools)} Playwright browser tools"
                                )
                                return mcp_tools
                            else:
                                ...
                                if not non_interactive:
                                    if self._prompt_yes_no(
                                        "Continue anyway? (login will fail without tools)",
                                        default=False,
                                    ):
                                        return None
                        else:
                        ...
                        if not non_interactive:
                            if self._prompt_yes_no(
                                "Continue anyway? (login will fail without tools)", default=False
                            ):
                                return None
                else:
                    print(
                        "⚠️  Playwright initialization skipped. MCP tools are required for browser automation."
                    )
                    if not non_interactive:
                        if self._prompt_yes_no(
                            "Continue anyway? (login will fail without tools)", default=False
                ...
                if non_interactive or self._prompt_yes_no(
                    "Inject tools from this server?", default=True
                ):
                    try:
                        ...
                        if inject_tools_from_mcp_server(selected_server):
                            ...
                            if mcp_tools:
                                print(
                                    f"✅ Successfully injected {len(mcp_tools)} MCP browser tools"
                                )
                                return mcp_tools
                            else:
                                ...
                                print(
                                    "   Cursor IDE browser tools are only available when running inside Cursor IDE."
                                )
                                if not non_interactive:
                                    if self._prompt_yes_no(
                                        "Continue anyway? (login will fail without tools)",
                    ...
                    print(
                        "⚠️  Tool injection skipped. MCP tools are required for browser automation."
                    )
                    if not non_interactive:
                        if self._prompt_yes_no(
                            "Continue anyway? (login will fail without tools)", default=False
                ...
                if not server_running:
                    ...
                    if non_interactive or self._prompt_yes_no(
                        f"Start server '{selected_server}' now?", default=True
                    ):
                        success = runner.start_server(server_name=selected_server)
                        if not success:
                ...
                # Inject tools
                if non_interactive or self._prompt_yes_no(
                    "Inject tools from server?", default=True
                ):
                    try:
                        from .inject_mcp_tools import inject_tools_from_mcp_server
    ...
    def cmd_auth_login(self, args):
        ...
        self._print_step_header(1, "MCP Tools Check", "Checking for MCP browser tools...")

        if skip_mcp_setup:
            mcp_tools = self._get_mcp_browser_tools()
            if not mcp_tools:
                self._error(
                    "MCP browser tools not available. Use interactive mode to set them up.", 3
                )
        else:
        ...
        if not mcp_tools:
            error_msg = (
                ...
                "  python3 -m grant_automation_cli.mcp_setup_cli configure\n\n"
        ...
        self._print_step_header(2, "Browser Initialization", "Ready to initialize browser.")
        ...
        if not non_interactive:
            ...
            cred_choice = self._prompt_choice(
                "Would you like to:", cred_choices, default=cred_choices[0]
            )
        ...
        print("Ready to navigate to: https://apply.4culture.org/login")
                ...
                json.dumps(
                    {
                        ...
                        "location": "sdk_cli.py:cmd_auth_login:before_type_username",
                        "message": "Before typing username (skipping find_element check)",
                        "data": {},
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "post-fix",
                            "hypothesisId": "H5",
                            "location": "sdk_cli.py:cmd_auth_login:username_typed",
                            "message": "Username typed successfully",
                            "data": {},
                            "timestamp": int(__import__("time").time() * 1000),
                        }
                    )
                    ...
                    json.dumps(
                        {
                            ...
                            "runId": "post-fix",
                            "hypothesisId": "H5",
                            "location": "sdk_cli.py:cmd_auth_login:username_failed",
                            "message": "Username typing failed",
                            "data": {},
                            "timestamp": int(__import__("time").time() * 1000),
                        }
                    )
        ...
        if success:
            ...
            if "login" in current_url.lower():
                ...
                if "login" in current_url.lower():
                    ...
                    try:
                        ...
                        if snapshot and isinstance(snapshot, dict):
                            ...
                            if (
                                ...
                                self._error(
                                    "Login failed - error message detected on page. Check credentials.",
                                    3,
                                )
                    except:
                        pass
                    ...
                    self._error(
                        "Login may have failed - still on login page. Check credentials or try manually.",
                        3,
                    )
        ...
        if self.output_format == "human":
            print("\nAvailable commands:")
            print("  1. List grants: python3 -m grant_automation_cli.sdk_cli browser snapshot")
            print(
                '  2. Start application: python3 -m grant_automation_cli.sdk_cli workflow full --grant-name "..."'
            )
            print("  3. View dashboard: python3 -m grant_automation_cli.sdk_cli report dashboard")

        if not non_interactive:
            next_choices = ["Keep browser open", "Close browser", "Run another command"]
            next_choice = self._prompt_choice(
                "Would you like to:", next_choices, default=next_choices[0]
            )
    ...
    def cmd_data_extract(self, args):
        ...
        self._output(
            {"grant_name": grant_name, "data": grant_data, "output_file": args.output},
            "Data Extraction",
        )
    ...
    def cmd_data_identify_docs(self, args):
        ...
        self._output(
            {"grant_name": grant_name, "documents": documents, "count": len(documents)},
        ...
        if args.use_narratives:
            ...
            if NarrativeGenerator:
                mapper.narrative_generator = NarrativeGenerator(
                    api_key=api_key, model=args.narrative_model or "gpt-4"
                )
    ...
    def cmd_narrative_generate(self, args):
        ...
        if args.data_file:
            ...
            docs_folder = args.docs_folder or "docs"
            extractor = DataExtractor(docs_folder)
            grant_data = extractor.extract_grant_data(
                args.grant_name, args.doc and [args.doc] or None
            )

        generator = NarrativeGenerator(api_key=api_key, model=args.model or "gpt-4")

        narrative = generator.generate_narrative(args.narrative_type, args.grant_name, grant_data)
    ...
    def cmd_browser_navigate(self, args):
        ...
        if success:
            ...
            self._output(
                {"status": "success", "url": args.url, "current_url": current_url}, "Navigation"
            )
        else:
    ...
    def cmd_browser_snapshot(self, args):
        ...
        self._output({"snapshot": snapshot, "output_file": args.output}, "Page Snapshot")
    ...
    def cmd_browser_find(self, args):
        ...
        self._output({"description": args.description, "element": element}, "Element Find")
    ...
    def cmd_browser_type(self, args):
        ...
        success = browser.type_text(
            args.description, args.text, element_ref=args.element_ref, slowly=args.slowly
        )
    ...
    def cmd_browser_screenshot(self, args):
        ...
        self._output({"status": "success", "screenshot_path": screenshot_path}, "Screenshot")
    ...
    def cmd_browser_wait(self, args):
        ...
        browser = MCPBrowserIntegration()
        browser.initialize_browser(mcp_tools)
        success = browser.wait_for(text=args.text, text_gone=args.text_gone, time_seconds=args.time)

        self._output({"status": "success" if success else "failed"}, "Wait")
    ...
    def cmd_form_start(self, args):
        ...
        self._output(
            {"status": "success" if success else "failed", "grant_name": args.grant_name},
            "Start Application",
        )
    ...
    def cmd_form_extract_structure(self, args):
        ...
        self._output(
            {"form_structure": form_structure, "output_file": args.output}, "Form Structure"
        )
    ...
    def cmd_form_validate(self, args):
        ...
        self._output(
            {"has_errors": len(errors) > 0, "errors": errors, "error_count": len(errors)},
            "Form Validation",
        )
    ...
    def cmd_workflow_full(self, args):
        ...
        workflow = AutomationWorkflow(
            ...
            mcp_tools=mcp_tools,
            db_path=args.db_path or "grants.db",
        )
    ...
    def cmd_database_add(self, args):
        ...
        db = GrantDatabase(args.db_path or "grants.db")
        ...
        self._output({"grant_id": grant_id, "grant_name": args.name}, "Add Grant")
    ...
    def cmd_database_update(self, args):
        ...
        self._output({"grant_id": args.grant_id, "status": args.status}, "Update Grant")
    ...
    def cmd_database_list(self, args):
        ...
        self._output(
            {"status": args.status or "all", "count": len(grants), "grants": grants}, "List Grants"
        )
    ...
    def cmd_report_generate(self, args):
        ...
        workflow = AutomationWorkflow(username="dummy", password="dummy", docs_folder="docs")

        report_path = args.output or "grant_report.md"
        workflow.generate_report(result, report_path)

        self._output({"report_path": report_path}, "Report Generation")
    ...
    def cmd_report_dashboard(self, args):
        ...
        self._output({"dashboard_path": output_path}, "Dashboard Generation")
...
def create_parser():
    ...
    login_parser.add_argument(
        "--skip-mcp-setup", action="store_true", help="Skip MCP setup (fail if tools not available)"
    )
    login_parser.add_argument(
        "--skip-verification", action="store_true", help="Skip verification steps"
    )
    ...
    instructions_parser = mapping_sub.add_parser(
        "create-instructions", help="Create field filling instructions"
    )
    ...
    browser_sub.add_parser("snapshot", help="Take page snapshot").add_argument(
    ...
    form_sub.add_parser("fill-all", help="Fill all fields").add_argument(
        "--instructions-file", required=True
    )
    ...
    db_sub.add_parser("timeline", help="Get grants by month").add_argument(
        "--db-path", help="Database path"
    )
