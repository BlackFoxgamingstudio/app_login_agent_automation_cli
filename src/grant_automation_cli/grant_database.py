...
class GrantDatabase:
    ...
    def _init_database(self):
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_grant_deadline ON grants(deadline_timestamp)"
        )
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_app_status ON application_status(grant_id, status)"
        )
    ...
    def add_grant(self, grant_data: Dict[str, Any]) -> int:
        ...
        if grant_data.get("deadline"):
            try:
                ...
                try:
                    deadline_ts = int(
                        datetime.strptime(grant_data["deadline"], "%B %d, %Y").timestamp()
                    )
                except:
                    pass
        ...
        for row in cursor.fetchall():
            ...
            if deadline_ts:
                ...
                if month_key not in grants_by_month:
                    grants_by_month[month_key] = {"month_name": month_name, "grants": []}
    ...
    def add_milestone(
        self, grant_id: int, milestone_name: str, due_date: str, notes: Optional[str] = None
    ):
