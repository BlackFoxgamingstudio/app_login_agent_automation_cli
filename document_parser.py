...
class DocumentParser:
    ...
    def _extract_organization_info(self, content: str) -> Dict[str, Any]:
        ...
        org_info = {
            ...
            'staff_size': self._extract_number(content, r'(\d+)\s*(?:staff|employees|team members)'),
    ...
    def _extract_project_info(self, content: str) -> Dict[str, Any]:
        ...
        project_info = {
            ...
            'goals': self._extract_list_items(content, r'(?:goal|objective|purpose)[s]?:', case_insensitive=True),
    ...
    def _extract_timeline_info(self, content: str) -> Dict[str, Any]:
        ...
        timeline_info = {
            'start_date': self._extract_date(content, r'start(?:ing|s)?\s*(?:date|on)?\s*:?\s*([A-Za-z]+\s+\d{1,2},?\s+\d{4})'),
            'end_date': self._extract_date(content, r'end(?:ing|s)?\s*(?:date|on)?\s*:?\s*([A-Za-z]+\s+\d{1,2},?\s+\d{4})'),
            'deadline': self._extract_date(content, r'deadline\s*:?\s*([A-Za-z]+\s+\d{1,2},?\s+\d{4})'),
    ...
    def _extract_section_value(self, content: str, pattern: str, section_name: str) -> Optional[str]:
    ...
    def _extract_list_items(self, content: str, pattern: str, case_insensitive: bool = False) -> List[str]:
