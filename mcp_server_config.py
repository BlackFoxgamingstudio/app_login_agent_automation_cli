    ...
    def _connect_playwright(self, config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                logger.warning("Playwright Python library not installed. Install with: pip install playwright && playwright install")
        ...
        try:
            ...
            try:
                with open('/Users/russellpowers/Grants/.cursor/debug.log', 'a') as f:
                    f.write(json.dumps({"sessionId":"debug-session","runId":"post-fix-v2","hypothesisId":"H2","location":"mcp_server_config.py:_create_playwright_tools:entry","message":"Creating Playwright tools","data":{},"timestamp":int(__import__('time').time()*1000)}) + '\n')
            except Exception as e:
                pass
            # #endregion
            ...
            try:
                with open('/Users/russellpowers/Grants/.cursor/debug.log', 'a') as f:
                    f.write(json.dumps({"sessionId":"debug-session","runId":"post-fix-v2","hypothesisId":"H2","location":"mcp_server_config.py:_create_playwright_tools:import_success","message":"Playwright imported successfully","data":{},"timestamp":int(__import__('time').time()*1000)}) + '\n')
            except Exception as e:
                pass
            # #endregion
            
            # Global Playwright context
            _playwright_context = {'playwright': None, 'browser': None, 'page': None, 'lock': threading.Lock()}
                ...
                with _playwright_context['lock']:
                    if _playwright_context['playwright'] is None:
                        try:
                            ...
                            _playwright_context['browser'] = _playwright_context['playwright'].chromium.launch(headless=False)
            ...
            def playwright_click(selector_or_description: str) -> bool:
                ...
                try:
                    ...
                    is_likely_selector = (selector_or_description.startswith('#') or 
                                        selector_or_description.startswith('.') or
                                        selector_or_description.startswith('[') or
                                        (len(selector_or_description.split()) == 1 and 
                                         not any(word in selector_or_description.lower() for word in ['button', 'link', 'login', 'sign', 'submit', 'click'])))
                    ...
                    keywords = [word for word in desc_lower.split() if word not in ['the', 'a', 'an', 'or', 'and', 'button', 'link', 'element']]
                    if not keywords:
                        ...
                        for i, approach in enumerate(form_submit_approaches, 1):
                            try:
                                ...
                                if result or i > 1:  # First approach returns bool, others don't
                                    logger.info(f"Submitted form using method {i}: {selector_or_description}")
                                    return True
                            except Exception as e:
                    ...
                    if "button" in desc_lower or "login" in desc_lower or "sign" in desc_lower or "submit" in desc_lower:
                        approaches = [
                            ...
                            lambda: page.get_by_role("button", name="login", exact=False).click(timeout=3000),
                            lambda: page.get_by_role("button", name="sign in", exact=False).click(timeout=3000),
                            lambda: page.get_by_role("button", name="Sign In", exact=False).click(timeout=3000),
                            lambda: page.get_by_role("button", name="Log In", exact=False).click(timeout=3000),
                            lambda: page.get_by_role("button", name="submit", exact=False).click(timeout=3000),
                            # Try by text content in button (case insensitive)
                            lambda: page.locator('button').filter(has_text=re.compile(r'login|sign.*in', re.I)).first.click(timeout=3000),
                            # Try with keywords
                            lambda: page.get_by_role("button", name=lambda name: name and any(kw in name.lower() for kw in keywords if len(kw) > 2), exact=False).first.click(timeout=3000),
                    ...
                    approaches = [
                        lambda: page.get_by_text(keywords[0] if keywords else selector_or_description, exact=False).first.click(timeout=2000),
                        lambda: page.get_by_text(selector_or_description, exact=False).first.click(timeout=2000),
                        lambda: page.locator(f'text={keywords[0] if keywords else selector_or_description}').first.click(timeout=2000),
                    ]
                    ...
                    if "link" in desc_lower:
                        try:
                            page.get_by_role("link", name=lambda name: name and any(kw in name.lower() for kw in keywords if len(kw) > 2), exact=False).first.click(timeout=2000)
            ...
            def playwright_fill(selector_or_description: str, text: str) -> bool:
                ...
                try:
                    ...
                    is_likely_selector = (selector_or_description.startswith('#') or 
                                        selector_or_description.startswith('.') or
                                        selector_or_description.startswith('[') or
                                        selector_or_description.startswith('input') or
                                        selector_or_description.startswith('button') or
                                        len(selector_or_description.split()) == 1 and 
                                        not any(word in selector_or_description.lower() for word in ['field', 'input', 'button', 'username', 'password', 'email']))
                    ...
                    if "username" in desc_lower or "email" in desc_lower:
                        ...
                        approaches = [
                            ...
                            lambda: page.locator('input[name*="user" i], input[name*="email" i]').first.fill(text),
                            lambda: page.locator('input[id*="user" i], input[id*="email" i]').first.fill(text),
                        ]
                        for approach in approaches:
                            try:
                                approach()
                                logger.info(f"Filled username/email field using {approach.__name__}")
                                return True
                            except:
                                continue
            ...
            try:
                with open('/Users/russellpowers/Grants/.cursor/debug.log', 'a') as f:
                    f.write(json.dumps({"sessionId":"debug-session","runId":"post-fix-v2","hypothesisId":"H2","location":"mcp_server_config.py:_create_playwright_tools:success","message":"Playwright tools created","data":{"tool_count":len(tools),"tool_keys":list(tools.keys())},"timestamp":int(__import__('time').time()*1000)}) + '\n')
            except Exception as e:
                pass
            # #endregion
