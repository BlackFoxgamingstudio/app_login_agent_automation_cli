...
class FormMapper:
    ...
    def __init__(self, config_path: Optional[str] = None, narrative_generator: Optional[Any] = None,
                 use_narratives: bool = False, narrative_config: Optional[Dict[str, Any]] = None):
        """
            ...
            try:
                with open(config_path, 'r') as f:
                    ...
                    if narrative_config_from_file:
                        self.narrative_config = {**self.narrative_config, **narrative_config_from_file}
    ...
    def map_data_to_fields(self, standardized_data: Dict[str, Any], 
                          grant_name: Optional[str] = None,
                          grant_data: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        """
            ...
            if data_key in self.field_mappings:
                ...
                if should_narrate:
                    ...
                    if narrative:
                        ...
                        logger.warning(f"Narrative generation failed for {field_name}, using original value")
                else:
                    mapping['value'] = self._format_value(data_value, mapping.get('field_type'))
                    mapping['narrative_generated'] = False
    ...
    def _should_use_narrative(self, field_name: str, data_value: Any, data_key: Optional[str] = None) -> bool:
        """
        Check if narrative generation should be used for this field.
    ...
    def _generate_narrative_for_field(self, field_name: str, data_key: str, 
                                     grant_name: Optional[str],
                                     grant_data: Optional[Dict[str, Any]],
                                     standardized_data: Dict[str, Any]) -> Optional[str]:
    ...
    def get_field_by_name(self, field_name: str, mapped_fields: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    def validate_mapped_fields(self, mapped_fields: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for field in mapped_fields:
            ...
            if field_type == 'email' and value:
                if '@' not in value:
                    validation_result['invalid_fields'].append(f"{field.get('field_name')}: invalid email")
                    validation_result['valid'] = False
            ...
            if field_type == 'number' and value:
                try:
                    ...
                    validation_result['invalid_fields'].append(f"{field.get('field_name')}: invalid number")
                    validation_result['valid'] = False
        
        return validation_result
    
    def handle_conditional_fields(self, mapped_fields: List[Dict[str, Any]], form_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    def create_field_filling_instructions(self, mapped_fields: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
