...
class AutomationWorkflow:
    ...
    def __init__(self, username: str, password: str, docs_folder: str, config_path: Optional[str] = None,
                 use_narratives: bool = False, openai_api_key: Optional[str] = None,
                 narrative_model: Optional[str] = None, mcp_tools: Optional[Dict[str, Any]] = None):
        """
        ...
        if use_narratives and NarrativeGenerator:
            try:
                ...
                api_key = (
                    openai_api_key or  # CLI argument (highest priority)
                    narrative_config.get('openai_api_key') or  # Config file
                    os.getenv('OPENAI_API_KEY')  # Environment variable
                )
                ...
                logger.warning(f"Failed to initialize narrative generator: {e}. Continuing without narratives.")
    ...
    def run_automation(self, grant_name: str, document_paths: Optional[List[str]] = None, 
        ...
        try:
            ...
            if not validation['valid']:
                result['errors'].extend([f"Missing required field: {f}" for f in validation['missing_required']])
                if not dry_run:
            ...
            narratives_generated = sum(1 for f in mapped_fields if f.get('narrative_generated', False))
            ...
            if not field_validation['valid']:
                result['errors'].extend([f"Invalid field: {f}" for f in field_validation['invalid_fields']])
                result['warnings'].extend([f"Missing optional field: {f}" for f in field_validation.get('missing_required', [])])
            ...
            if self.mcp_tools:
                ...
                logger.info(f"Form structure extracted: {len(form_structure.get('fields', []))} fields found")
    ...
    def _update_database(self, grant_name: str, grant_data: Dict[str, Any],
                        mapped_fields: List[Dict[str, Any]], result: Dict[str, Any]):
        """Update database with grant information."""
        if not self.db:
            return
    ...
    def generate_report(self, result: Dict[str, Any], output_path: Optional[str] = None) -> str:
        ...
        if narratives_generated > 0:
            ...
            narrative_fields = [f for f in result.get('fields_mapped', []) if f.get('narrative_generated', False)]
            if narrative_fields:
                report_lines.append("")
