...
class GrantApplicationCLI:
    ...
    def _run_automate(self, args):
        ...
        workflow = AutomationWorkflow(
            username=args.username or "Keyisaccess",
            password=args.password or "Outlaw22!!",
        ...
        if args.use_narratives:
            logger.info("Narrative generation enabled - will convert bullet points to complete prose")
    ...
    def _run_dashboard(self, args):
        ...
        print(f"\n   The dashboard shows:")
        print(f"   - All planned grant applications")
        print(f"   - Applications in progress with completion status")
        print(f"   - Submitted and completed applications")
        print(f"   - Grants organized by deadline month")
        print(f"   - Priority-based view for strategic planning")
...
Examples:
  # Extract data only
  python3 -m agents.grant_scraper.cli extract --grant "Moving Stories" --doc docs/02-green-stem-expo-technical.md
  
  # Validate extracted data
  python3 -m agents.grant_scraper.cli validate --grant "Moving Stories"
  
  # Run automation (dry run)
  python3 -m agents.grant_scraper.cli automate --grant "Moving Stories" --dry-run
  
  # Run full automation
  python3 -m agents.grant_scraper.cli automate --grant "Moving Stories" --username Keyisaccess --password "Outlaw22!!"
...
Note: For developers needing fine-grained control over individual steps, use the SDK CLI:
  python3 -m agents.grant_scraper.sdk_cli --help
    ...
    automate_parser.add_argument('--username', help='4Culture username (default: Keyisaccess)')
    automate_parser.add_argument('--password', help='4Culture password')
    automate_parser.add_argument('--dry-run', action='store_true', help='Extract data but don\'t fill form')
    automate_parser.add_argument('--report', help='Path to save completion report')
    automate_parser.add_argument('--use-narratives', action='store_true', help='Use OpenAI to generate complete narratives (no bullet points)')
    automate_parser.add_argument('--openai-api-key', help='OpenAI API key for narrative generation')
    automate_parser.add_argument('--narrative-model', help='OpenAI model to use for narratives (default: gpt-4)')
    
    # Dashboard command
    dashboard_parser = subparsers.add_parser('dashboard', help='Generate HTML grant planning dashboard')
    dashboard_parser.add_argument('--db', default='agents/grant_scraper/grants.db', help='Path to grant database')
